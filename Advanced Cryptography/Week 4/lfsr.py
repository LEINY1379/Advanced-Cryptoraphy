...
class LFSR:
  def __init__(self, seed_bits, tap_positions):
    ...
    self.initial_state = list(seed_bits)
...
def generate(self, stream_length):
  return [self.step() for _ in range(stream_length)]
...
if __name__ == "__main__":
  ...
  lfsr = LFSR(seed_bits=starting_seed, tap_positions=feedback_taps)

  bit_stream = lfsr.generate(50)
  print(f"Generated bits: {bit_stream}")
  print(f"Bit string:    {''.join(map(str, bit_stream))}")

  period, start_loop = lfsr.find_sequence_period()
  print(f"Period deteected: {period} (starts repeating at step {start_loop})")
